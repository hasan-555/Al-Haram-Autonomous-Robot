...
class DiffDriveModbusNode(Node):
    ...
    def cmd_vel_callback(self, msg):
        """
        Callback for /cmd_vel topic. Extracts linear.x (v in m/s) and angular.z (w in rad/s),
        converts to cm/s and mrad/s, and processes the velocity command.
        """
        v = msg.linear.x   # Convert m/s to cm/s
        w = msg.angular.z  # Convert rad/s to mrad/s
...
def main(args=None):
    ...
    try:
        rclpy.spin(node)  # Keep node running to process subscriptions
