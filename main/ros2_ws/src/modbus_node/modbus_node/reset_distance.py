...
class ModbusReadNode(Node):
    def __init__(self):
        ...
        # Reset target positions to zero
    ...
    def reset_target_positions(self):
        """Write zero to target position registers for left (2084h, 208Bh) and right (208Ch, 208Dh) motors."""
        if not self._check_serial():
            self.get_logger().error("Cannot reset target positions: Serial port is not open")
            return

        # Command to write zero to left motor (2084h, 208Bh) and right motor (208Ch, 208Dh)
        # Using function code 0x10 to write multiple registers
        # Writing 4 registers starting at 2084h: 0x0000 0x0000 (left high/low), 0x0000 0x0000 (right high/low)
        command = bytes([
            0x01,  # Driver Address
            0x10,  # Function Code (Write Multiple Registers)
            0x20, 0x84,  # Starting address (2084h)
            0x00, 0x04,  # Number of registers (4)
            0x08,  # Number of data bytes (4 registers * 2 bytes)
            0x00, 0x00,  # Left high 16 bits (2084h)
            0x00, 0x00,  # Left low 16 bits (208Bh)
            0x00, 0x00,  # Right high 16 bits (208Ch)
            0x00, 0x00   # Right low 16 bits (208Dh)
        ])
        command += self._calculate_crc(command)

        for attempt in range(self.retry_count):
            try:
                self.get_logger().info(f"Sending reset position command (attempt {attempt + 1}/{self.retry_count}): {command.hex()}")
                self.ser.reset_input_buffer()
                self.ser.write(command)
                
                # Expect 8 bytes response: 1 addr + 1 func + 2 start addr + 2 reg count + 2 CRC
                response = self.ser.read(8)
                self.get_logger().info(f"Received response: {response.hex()}")
                
                if len(response) == 8 and response[0] == 0x01 and response[1] == 0x10:
                    self.get_logger().info("Successfully reset target positions to zero")
                    return
                else:
                    self.get_logger().warn(f"Invalid response length or format: {len(response)} bytes")
            except serial.SerialException as e:
            ...
            time.sleep(0.2)
        self.get_logger().error("Failed to reset target positions after all retries")
