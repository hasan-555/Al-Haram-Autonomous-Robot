...
class ModbusReadNode(Node):
    def __init__(self):
        ...
        # Create publisher for /wheel/odom topic
        self.odom_pub = self.create_publisher(Point, '/wheel/odom', 10) # x for left motor , y for right motor
    ...
    def read_modbus_registers(self):
        """Read motor positions, calculate distances, and publish to /wheel/odom every 500ms."""
        if not self._check_serial():
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                if len(response) == 13:
                    ...
                    # Publish distances to /wheel/odom topic
                    odom_msg = Point()
                    odom_msg.x = left_distance  # Left wheel distance in meters
                    odom_msg.y = right_distance  # Right wheel distance in meters
                    ...
                    self.get_logger().info(
                        ...
                        f"Published to /wheel/odom: left={left_distance:.4f} m, right={right_distance:.4f} m"
                    )
                    return
                else:
